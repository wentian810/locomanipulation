...
PIPELINE_ROOT="${PIPELINE_ROOT:-$(cd "$HERE/../.." && pwd)}"
OBJECT_MESH_BACKEND="${OBJECT_MESH_BACKEND:-sam3d}"
HUNYUAN3D_FACE_COUNT="${HUNYUAN3D_FACE_COUNT:-100000}"
HUNYUAN3D_TIMEOUT="${HUNYUAN3D_TIMEOUT:-900}"
HUNYUAN3D_REGION="${HUNYUAN3D_REGION:-ap-guangzhou}"
OBJECT_NUM_POSE_SAMPLES="${OBJECT_NUM_POSE_SAMPLES:-25}"
OBJECT_EULER_STEPS="${OBJECT_EULER_STEPS:-25}"
OBJECT_TORCH_COMPILE="${OBJECT_TORCH_COMPILE:-1}"
...
# Remove only the final media extension. Dataset clip names legitimately contain
# dots (for example ``...hand.0.color.mp4``); ``%%.*`` truncated them at the
# first dot and made the HaWoR output directory impossible to rediscover.
...
if [ "$OBJECT_MESH_BACKEND" = "hunyuan" ]; then
    for OBJ_NAME in "${OBJECT_NAMES[@]}"; do
        OBJECT_ID="${OBJ_NAME// /_}"
        OBJECT_ASSET_DIR="$MASKS_DIR/$OBJECT_ID"
        RGBA_PATH="$OBJECT_ASSET_DIR/${OBJECT_ID}_rgba.png"
        GLB_PATH="$OBJECT_ASSET_DIR/${OBJECT_ID}.glb"
        OBJ_PATH="$OBJECT_ASSET_DIR/${OBJECT_ID}.obj"
        mkdir -p "$OBJECT_ASSET_DIR"

        python "$SCRIPTS_DIR/prepare_hunyuan_rgba.py" \
            --image "$FRAME_PATH" \
            --mask "$MASKS_DIR/$OBJECT_ID.png" \
            --output "$RGBA_PATH"
        python "$PIPELINE_ROOT/foundationpose-plus-plus-main/scripts/hunyuan3d_bridge.py" \
            --rgba_path "$RGBA_PATH" \
            --output_mesh "$GLB_PATH" \
            --face_count "$HUNYUAN3D_FACE_COUNT" \
            --region "$HUNYUAN3D_REGION" \
            --poll_timeout "$HUNYUAN3D_TIMEOUT"
        python "$PIPELINE_ROOT/foundationpose-plus-plus-main/scripts/glb_to_obj.py" \
            --glb_path "$GLB_PATH" \
            --output "$OBJ_PATH"
    done
elif [ "$OBJECT_MESH_BACKEND" = "sam3d" ]; then
    cd "$SAM3D_DIR"                                  # overlay: generate_mesh_sam3d.py
    python generate_mesh_sam3d.py \
        --image_path "$FRAME_PATH" \
        --masks_dir "$MASKS_DIR"
else
    echo "Unsupported OBJECT_MESH_BACKEND=$OBJECT_MESH_BACKEND (use sam3d|hunyuan)" >&2
    exit 2
fi
...
for OBJ_NAME in "${OBJECT_NAMES[@]}"; do
    OBJECT_ID="${OBJ_NAME// /_}"
    FAST_SAM3D_COMPILE_ARGS=()
    if [ "$OBJECT_TORCH_COMPILE" = "1" ]; then
        FAST_SAM3D_COMPILE_ARGS+=(--torch_compile)
    fi
    python track_object.py \
        ...
        --num_pose_samples "$OBJECT_NUM_POSE_SAMPLES" \
        ...
        --enable_ss_cache \
        "${FAST_SAM3D_COMPILE_ARGS[@]}" \
        --euler_steps "$OBJECT_EULER_STEPS" \
