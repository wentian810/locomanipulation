...
def robot_geom_lowest_heights(xml_file, root_pos, root_rot_xyzw, dof_pos):
...
def load_npz_body_motion(npz_path, body_model_path, model_type, hand_npz=None):
    ...
    if left_hand is not None and left_hand.shape[0] >= num_frames:
        left_hand_pose = left_hand[:num_frames]
    else:
        left_hand_pose = torch.zeros(num_frames, left_hand_dim).float()

    if right_hand is not None and right_hand.shape[0] >= num_frames:
        right_hand_pose = right_hand[:num_frames]
    else:
        right_hand_pose = torch.zeros(num_frames, right_hand_dim).float()
...
def resolve_hand_npz_for_source(src_file, args):
    ...
    candidate = real_dir / args.hand_npz_name
    if candidate.is_file():
        return candidate

    # Fallback: look beside the unresolved path (e.g. auto-src symlink target)
    candidate = src_file.parent / args.hand_npz_name
...
def wrist_pitch_yaw_stabilization_enabled(args):
    ...
    if mode == "on":
        return True
    if mode == "auto":
        return args.robot in WRIST_PITCH_YAW_STABILIZE_ROBOTS and not wrist_orientation_override_enabled(args)
...
def compute_palm_roll_delta(
    ...
    if source in {"auto", "sidecar_joints"}:
        ...
        if sidecar_normal is not None and sidecar_valid is not None and int(sidecar_valid.sum()) >= 2:
            ...
            return None, int(sidecar_valid.sum()) if sidecar_valid is not None else 0, "sidecar_joints_missing", rejected_jumps
    ...
    if palm_normal is None and source in {"auto", "sidecar"}:
        ...
        if sidecar_normal is not None and sidecar_valid is not None and int(sidecar_valid.sum()) >= 2:
            ...
            return None, int(sidecar_valid.sum()) if sidecar_valid is not None else 0, "sidecar_missing", rejected_jumps
    ...
    if palm_normal is None:
        ...
        if any(idx is None for idx in smpl_indices):
            return None, 0, "missing_smpl_hand", rejected_jumps
    ...
    if source_used == "smpl_mcp":
        ...
        if hand_metadata is not None and wrist_valid_key in hand_metadata:
            valid &= np.asarray(hand_metadata[wrist_valid_key], dtype=bool).reshape(-1)[:frame_count]
    if np.sum(valid) < 2:
        return None, int(np.sum(valid)), source_used, rejected_jumps
    ...
    return delta.astype(np.float32), int(np.sum(valid)), source_used, rejected_jumps
...
def apply_palm_roll_to_wrist_joints(dof_pos, model, smplx_joints, joint_names, hand_metadata, args):
    ...
    for side in ("left", "right"):
        ...
        delta, valid_frames, source_used, rejected_jumps = compute_palm_roll_delta(
...
def convert_file(src_file, tgt_file, args, device, hand_npz=None):
        ...
        if args.verbose:
            for side in ("left", "right"):
                ...
                print(
                    ...
                    f"jumps_rejected={rejected}, branch_switches={switches}, "
    ...
    if args.height_adjust_mode in {"per_frame_geom", "global_geom"}:
        lowest_height = robot_geom_lowest_heights(retargeter.xml_file, root_pos, root_rot, dof_pos)
        if args.height_adjust_mode == "per_frame_geom":
...
def main():
    ...
    parser.add_argument(
        "--height_adjust_mode",
        choices=["global", "per_frame", "global_geom", "per_frame_geom", "none"],
        default="global",
        help=(
    ...
    parser.add_argument(
        ...
        choices=["auto", "on", "off"],
        default="auto",
        help="Lock relaxed G1 wrist pitch/yaw to neutral so palm roll does not create a broken wrist angle.",
    )
    parser.add_argument(
        "--wrist_pitch_neutral",
        default=0.0,
