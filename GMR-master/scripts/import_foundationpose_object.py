...
def _load_intrinsics(path: Path | None, foundation_dir: Path, fallback) -> np.ndarray:
    ...
    if selected.suffix.lower() == ".npy":
        return np.asarray(np.load(selected), dtype=np.float32)
...
def export(args) -> None:
    ...
    pose_path = (args.pose_path or foundation_dir / "pose.npy").resolve()
    ...
    poses_camera = np.asarray(np.load(pose_path), dtype=np.float64)
    _validate_poses(poses_camera, "FoundationPose pose.npy")
    ...
    if pose_valid_path.exists():
        pose_valid = np.asarray(np.load(pose_valid_path), dtype=bool).reshape(-1)
    ...
    with np.load(camera_path, allow_pickle=True) as camera_npz:
    ...
    source_fps = float(args.source_fps or robot_fps)
    source_positions = (
