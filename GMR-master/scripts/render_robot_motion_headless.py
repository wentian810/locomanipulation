...
def ensure_sharpa_assets(target_root, sharpa_root):
...
def build_h1_sharpa_visual_xml(
    base_xml,
    sharpa_root,
    mount_pos,
    left_mount_quat,
    ...
    root = tree.getroot()
    ensure_sharpa_assets(root, sharpa_root)
    ...
    for side in ("left", "right"):
        ...
        body.set("pos", " ".join(f"{v:.8g}" for v in mount_pos))
        ...
        if parent is None:
            raise ValueError(f"{side}_hand_link not found in {base_xml}")
        parent.append(body)
...
def hide_original_h1_hands(model):
    hand_ids = {
        "left": model.body("left_hand_link").id,
        "right": model.body("right_hand_link").id,
    }
    sharpa_ids = {
...
def render_skeleton(args):
    ...
    writer = imageio.get_writer(args.video_path, fps=video_fps)
...
def render_mujoco(args):
    ...
    _, fps, root_pos, root_rot_xyzw, dof_pos, _, _ = load_motion(args.robot_motion_path)
