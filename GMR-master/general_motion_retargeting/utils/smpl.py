...
def load_hand_poses_from_sidecar(hand_npz_path, missing_ok=True):
    """Load left_hand_pose and right_hand_pose from a sidecar NPZ.

    Returns (left_hand_pose, right_hand_pose, left_valid, right_valid) as
    torch tensors, or (None, None, None, None) if the path is missing.
    """
    if hand_npz_path is None:
        return None, None, None, None

    hand_path = Path(hand_npz_path).expanduser()
    if not hand_path.is_file():
        if missing_ok:
            return None, None, None, None
        raise FileNotFoundError(f"hand sidecar NPZ not found: {hand_path}")

    with np.load(hand_path, allow_pickle=True) as data:
        left = torch.from_numpy(np.asarray(data["left_hand_pose"], dtype=np.float32)).float()
        right = torch.from_numpy(np.asarray(data["right_hand_pose"], dtype=np.float32)).float()
        left_valid_np = (
            np.asarray(data["left_hand_valid"], dtype=bool)
            if "left_hand_valid" in data.files
            else np.ones(left.shape[0], dtype=bool)
        )
        right_valid_np = (
            np.asarray(data["right_hand_valid"], dtype=bool)
            if "right_hand_valid" in data.files
            else np.ones(right.shape[0], dtype=bool)
        )
    left_valid = torch.from_numpy(left_valid_np).bool()
    right_valid = torch.from_numpy(right_valid_np).bool()
    return left, right, left_valid, right_valid


def load_hand_metadata_from_sidecar(hand_npz_path, missing_ok=True):
    """Load optional hand orientation diagnostics from a sidecar NPZ."""
    if hand_npz_path is None:
        return {}
    ...
    optional_keys = (
        ...
        "left_hand_quality",
        "right_hand_quality",
        "left_hand_source_reliable",
        "right_hand_source_reliable",
        "left_hand_source_repaired",
        "right_hand_source_repaired",
    )
    out = {}
    with np.load(hand_path, allow_pickle=True) as data:
        for key in optional_keys:
            ...
            elif value.dtype.kind in {"f", "i", "u"}:
                value = value.astype(np.float32)
            out[key] = value
    return out


def load_smplx_file(smplx_file, smplx_body_model_path, hand_npz=None):
