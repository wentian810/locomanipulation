...
class KalmanFilter6D(object):
    ...
    def __init__(self, measurement_noise_scale):
        ...
        self._std_weight_trans_xy = 1./15  # xy sensor has lower noise floor
        # Process noise parameters
        self._std_weight_trans = 1./10    # Translation uncertainty
        self._std_weight_rot = 1./20     # Rotation uncertainty
        self._std_weight_vel_trans = 1./15     # Translation speed uncertainty
        self._std_weight_vel_rot = 1./40    # Rotation speed uncertainty

        self.measurement_noise_scale = measurement_noise_scale
        self._base_std_weight_trans = self._std_weight_trans
        self._base_std_weight_vel_trans = self._std_weight_vel_trans
        self._base_std_weight_vel_rot = self._std_weight_vel_rot

    def set_process_noise_scale(self, scale: float):
        """动态调整过程噪声.
        """
        self._std_weight_vel_trans = self._base_std_weight_vel_trans * scale
        self._std_weight_vel_rot = self._base_std_weight_vel_rot * scale
    ...
    def project_for_xy(self, mean, covariance):
        scale_xy = max(np.linalg.norm(mean[:2]), abs(mean[2]) * 0.2, 1e-5)
