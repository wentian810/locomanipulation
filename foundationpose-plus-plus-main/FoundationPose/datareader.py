...
class YcbineoatReaderCustom:
  def __init__(
    ...
    self.zfar = zfar
    self.color_files = sorted(glob.glob(f"{rgb_dir}/*.jpg"))
    self.K = cam_K # shape=3*3
    self.id_strs = []
    ...
    self.gt_pose_files = sorted(glob.glob(f'{gt_pose_dir}/*'))
  ...
  def get_video_name(self):
    return self.episode_index, self.camera_index
  ...
  def get_mask(self,i):
    mask = cv2.imread(self.color_files[i].replace('color','masks'),-1)
    if len(mask.shape)==3:
      for c in range(3):
        if mask[...,c].sum()>0:
          mask = mask[...,c]
  ...
  def get_depth(self,i):
    depth = cv2.imread(self.color_files[i].replace('color','depth').replace('.jpg', '.png'),-1)/1e3
  ...
  def get_occ_mask(self,i):
    hand_mask_file = self.color_files[i].replace('color','masks_hand')
    ...
    right_hand_mask_file = self.color_files[i].replace('color','masks_hand_right')
