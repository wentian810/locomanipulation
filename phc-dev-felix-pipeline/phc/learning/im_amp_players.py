...
class IMAMPPlayerContinuous(amp_players.AMPPlayerContinuous):
    def __init__(self, config):
        ...
        self.seq_export_states = bool(getattr(humanoid_env.cfg, "seq_export_states", False))
        if self.seq_export_states:
            print("Sequential export mode enabled")
            humanoid_env.recording = True
            humanoid_env.recording_state_change = False
            humanoid_env._clear_recorded_states()
    ...
    def _post_step(self, info, done):
        ...
        if flags.im_eval:
            ...
            if self.curr_stpes >= curr_max or self.terminate_state.sum() == humanoid_env.num_envs:
                ...
                if self.seq_export_states:
                    print(f"Exporting repaired motion for start_idx={humanoid_env.start_idx}")
                    humanoid_env.export_repaired_motion_for_current_sample()
                ...
                if (humanoid_env.start_idx + humanoid_env.num_envs >= humanoid_env._motion_lib._num_unique_motions):
                    ...
                    print("....")
                    if self.seq_export_states:
                        humanoid_env._cleanup_and_exit()
