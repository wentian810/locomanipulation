...
def hand_observation_quality(mano, side, people, frames, args):
    """Continuous WiLoR evidence quality used only by the P2 gated mode."""

    valid = as_numpy(
        mano.get(f"{side}_hand_valid", np.ones((people, frames), dtype=bool))
    ).astype(bool)
    if valid.ndim == 1 and people == 1:
        valid = valid[None]
    if valid.shape != (people, frames):
        raise ValueError(f"Unexpected {side} hand valid shape: {valid.shape}")

    quality = valid.astype(np.float64)
    error_key = f"{side}_hand_reproj_error"
    if error_key in mano:
        error = as_numpy(mano[error_key]).astype(np.float64)
        if error.ndim == 1 and people == 1:
            error = error[None]
        if error.shape != (people, frames):
            raise ValueError(f"Unexpected {error_key} shape: {error.shape}")
        finite = np.isfinite(error) & (error < 1e5)
        denom = max(float(args.reproj_bad_px) - float(args.reproj_good_px), 1e-6)
        reproj = np.zeros_like(quality)
        reproj[finite] = np.clip(
            (float(args.reproj_bad_px) - error[finite]) / denom, 0.0, 1.0
        )
        quality *= reproj

    bbox_key = f"{side}_hand_bbox_xyxy"
    if bbox_key in mano:
        bbox = as_numpy(mano[bbox_key]).astype(np.float64)
        if bbox.ndim == 2 and people == 1:
            bbox = bbox[None]
        if bbox.shape != (people, frames, 4):
            raise ValueError(f"Unexpected {bbox_key} shape: {bbox.shape}")
        diag = np.linalg.norm(bbox[..., 2:] - bbox[..., :2], axis=-1)
        finite = np.isfinite(diag) & (diag > 0.0)
        denom = max(float(args.bbox_saturate) - float(args.bbox_min_diag), 1e-6)
        bbox_quality = np.zeros_like(quality)
        bbox_quality[finite] = np.clip(
            (diag[finite] - float(args.bbox_min_diag)) / denom, 0.0, 1.0
        )
        quality *= bbox_quality
...
def viterbi_filter(
    ...
    for t in range(frame_count):
        ...
        # A body wrist is a useful fallback, not an excuse to alter good
        # WiLoR observations.  The legacy selector had only a bool valid mask,
        # so it could choose ``mix`` even on high-quality frames.  P2 supplies
        # continuous reprojection/bbox evidence and gates those alternatives.
        if quality is not None and quality[t] >= float(quality_threshold):
            unary[t, 1:] += 1e6
...
def filter_side(mano, smpl_params, side, args):
    ...
    quality_all = (
        hand_observation_quality(mano, side, hamer.shape[0], hamer.shape[1], args)
        if args.quality_gated
        else None
    )
    ...
    for person_idx in range(people):
        ...
        fixed, path = viterbi_filter(
            ...
            quality=None if quality_all is None else quality_all[person_idx],
            quality_threshold=args.body_quality_threshold,
        )
    ...
    stats = {
        ...
        f"{side}_quality_gated": bool(args.quality_gated),
        f"{side}_low_quality_frames": int(
            np.count_nonzero(quality_all < float(args.body_quality_threshold))
        ) if quality_all is not None else 0,
    }
...
def main():
    ...
    parser.add_argument(
        "--quality_gated",
        action="store_true",
        help="Allow body/flip/mix wrist candidates only when continuous WiLoR evidence is low.",
    )
    parser.add_argument("--body_quality_threshold", type=float, default=0.70)
