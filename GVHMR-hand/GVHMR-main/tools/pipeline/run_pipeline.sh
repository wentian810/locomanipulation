...
ground_fix_npz() {
    ...
    if "$PY_PHC" "${LEGACY_PIPELINE}/fix_motion_floor.py" \
...
if [ "$SKIP_EXISTING" = "1" ] && [ "$FORCE_LOCO" != "1" ] && [ "$FORCE_SMOOTH" != "1" ] && [ -f "$SMOOTH_NPZ" ]; then
    ...
    if "$PY_PHC" "${LEGACY_PIPELINE}/smooth_motion.py" \
...
if [ "$SKIP_PHC" = "1" ]; then
    ...
    if "$PY_PHC" scripts/data_process/batch_repair_zitai.py \
...
if [ "$RENDER_COMPARISON" = "1" ]; then
    ...
    "$PY_PHC" "${LEGACY_PIPELINE}/render_comparison.py" \
        --video "$VIDEO" \
