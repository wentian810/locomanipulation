...
def match_last_dim(value, target_dim):
    if value.shape[-1] == target_dim:
        return value
    if value.shape[-1] > target_dim:
        return value[..., :target_dim].contiguous()
    pad_shape = (*value.shape[:-1], target_dim - value.shape[-1])
    return torch.cat([value, value.new_zeros(pad_shape)], dim=-1)
...
class BodyModelSMPLX(nn.Module):
    ...
    def __init__(self, model_path, **kwargs):
        ...
        self.shape_dim = int(self.bm.num_betas)

        # For fast computing of skeleton under beta
        shapedirs = self.bm.shapedirs[:, :, : self.shape_dim]
    ...
    def forward(
        ...
        **kwargs
        ...
        if betas is None:
            betas = torch.zeros([batch_size, self.shape_dim], dtype=dtype, device=device)
        if transl is None:
            transl = torch.zeros([batch_size, 3], dtype=dtype, device=device)
        betas = match_last_dim(betas, self.shape_dim)
        expression = match_last_dim(expression, self.bm.num_expression_coeffs)
    ...
    def get_skeleton(self, betas):
        """betas: (*, D) -> skeleton_beta: (*, 22, 3)"""
        betas = match_last_dim(betas, self.J_shapedirs.shape[-1])
